...
class Main_SMG:
    ...
    def main_SMG(self):
        ...
        begin = time.time()
#         print "begin", len(f1_value_list), len(f2_value_list)
...
#                         print "before", len(f1_value_list), len(f2_value_list)
...
#                         print "after", len(f1_value_list), len(f2_value_list)
...
### when loss is not involved, will be deleted later
class Main_SMG_m2:
    ## general parameters setting
    epsion = 1e-4
    max_iter = 1000
    ...
    def LSMG(self, x_k, num_steps, flag):
        ...
        return self.func.f1(x_k.reshape(1,-1)), self.func.f2(x_k.reshape(1,-1)), x_k
...
#                     print np.linalg.norm(G,axis = 1)
    ...
    def remove_dense(self, list_f1, list_f2, list_f3, list_pt):
        ...
        keep_index_f1 = np.unique(keep_index_f1)
        keep_index_f2 = np.unique(keep_index_f2)
        keep_index_f3 = np.unique(keep_index_f3)
        
        keep_index = np.union1d(index_f1[keep_index_f1], index_f2[keep_index_f2])
        keep_index = np.union1d(keep_index, index_f3[keep_index_f3])
